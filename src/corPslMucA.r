...
##### mucA gene between intact and non-intact psl operon #####
mucA = as.character(read.FASTA(paste0(pT[3],f$fNam[which(f$gEne=="PA0763")]), type = "DNA"))
...
write.FASTA(as.DNAbin(seq.pslIntact), paste0(pT[1],"PA0763-pslIntact.fa"))
write.FASTA(as.DNAbin(seq.pslLoss), paste0(pT[1],"PA0763-pslLoss.fa"))

## Seq alignment between two groups
cat(date(),": Multiple Sequence Alignment - psl partial loss in CF-associated strains\n")
seq.pslLoss = msa(readDNAStringSet(paste0(pT[1],"PA0763-pslLoss.fa")), method = "ClustalW")
cat(date(),": Multiple Sequence Alignment - psl intact in CF-associated strains\n")
seq.pslIntact = msa(readDNAStringSet(paste0(pT[1],"PA0763-pslIntact.fa")), method = "ClustalW")
cat(date(),"MSA done\n")

cQ.L = msaConsensusSequence(seq.pslLoss)
cQ.I = msaConsensusSequence(seq.pslIntact)

write.FASTA(as.DNAbin(msaConvert(seq.pslIntact, type = "seqinr::alignment")), paste0(pT[1],"PA0763-pslIntact.fa"))
write.FASTA(as.DNAbin(msaConvert(seq.pslLoss, type = "seqinr::alignment")), paste0(pT[1],"PA0763-pslLoss.fa"))

## Plot nucleotide conservation
sL = as.data.frame(as.character(read.FASTA(paste0(pT[1],"PA0763-pslLoss.fa"), type = "DNA")))
sI = as.data.frame(as.character(read.FASTA(paste0(pT[1],"PA0763-pslIntact.fa"), type = "DNA")))
...
jpeg(paste0(pT[2],"mucA_pslIntactness.jpeg"), width = 2000, height = 1500, res = 300)
par(mar = c(5,4,0,1)+.1)
...
legend("topright", legend = capFirst(seq.c), fill = cBp)
invisible(dev.off())
...
write.csv(mucA.dNdS[[2]], paste0(pT[2],"mucA_pslIntactness.csv"), row.names = F, quote = F)
