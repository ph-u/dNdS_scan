...
par(mar = c(4,4,1,1)+.1, mfrow = c(5,1), cex.axis = 1.2)

matplot(x = row.names(p0), y = p0[,-(1:2)], type = "b", pch = stP:(stP-1+length(pRe)), col = cBp[1:length(pRe)], xlab = paste0(argv[1]," Residue position"), ylab = "Single codon resolution 1st-quartile dN/dS", cex = .7)
abline(h=1, col = "#000000aa", lty = 3, lwd = 2)

matplot(x = row.names(p1), y = p1[,-(1:2)], type = "b", pch = stP:(stP-1+length(pRe)), col = cBp[1:length(pRe)], xlab = paste0(argv[1]," Residue position"), ylab = "Single codon resolution mean dN/dS", cex = .7)
abline(h=1, col = "#000000aa", lty = 3, lwd = 2)

matplot(x = row.names(p2), y = p2[,-(1:2)], type = "b", pch = stP:(stP-1+length(pRe)), col = cBp[1:length(pRe)], xlab = paste0(argv[1]," Residue position"), ylab = "Single codon resolution median dN/dS", cex = .7)
abline(h=1, col = "#000000aa", lty = 3, lwd = 2)

matplot(x = row.names(p3), y = p3[,-(1:2)], type = "b", pch = stP:(stP-1+length(pRe)), col = cBp[1:length(pRe)], xlab = paste0(argv[1]," Residue position"), ylab = "Single codon resolution 3rd-quartile dN/dS", cex = .7)
abline(h=1, col = "#000000aa", lty = 3, lwd = 2)
