...
fNam = strsplit(sub("^00_","",sub("_db.fa","",argv[1])),"_")[[1]]
fNam = c(paste0(fNam[-length(fNam)], collapse = "_"),fNam[length(fNam)])
oNam = paste0(pT[1], paste0(fNam, collapse = "_"), "--dbSum.csv")

fLank = read.csv(paste0(pT[1],fNam[1],"-flanking.csv"), header = F)
...
## flanking region similarity
        ...
        db.f = as.character(read.FASTA(paste0(pT[2], db.nam[1]), type="DNA"))
